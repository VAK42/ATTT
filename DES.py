...
def encrypt(pt, rkb, rk):
  ...
  for i in range(0, 16):
    ...
    sboxStr = ""
    for j in range(0, 8):
      row = bin2dec(int(xorX[j * 6] + xorX[j * 6 + 5]))
      col = bin2dec(
        int(xorX[j * 6 + 1] + xorX[j * 6 + 2] + xorX[j * 6 + 3] +
          xorX[j * 6 + 4]))
      val = sbox[j][row][col]
      sboxStr = sboxStr + dec2bin(val)
...
rkbRev = rkb[::-1]
rkRev = rk[::-1]
text = bin2hex(encrypt(cipherText, rkbRev, rkRev))
